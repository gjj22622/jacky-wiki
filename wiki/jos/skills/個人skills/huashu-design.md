...
| **吉祥物 / Mascot** | 极高 · 有吉祥物的品牌，mascot 就是品牌人格化身（Duolingo 猫头鹰、Mailchimp 黑猩猩、LINE Friends、GEICO 壁虎、米其林宝宝）| **凡品牌设有 mascot 必须用真实素材；禁止 SVG 手画代替** |
...
**Mascot 警示**：吉祥物是品牌「人格代言」。SVG 手画 mascot 必然失真——比例、表情、配色、姿态任何一项错都会让动画「不像那个品牌」。手画版本就算技术上画得不错，**物种/造型/姿态错了就是另一个品牌**。**有官方 mascot 就用真实 PNG/SVG；没有就别加 mascot；不要发明 mascot**。

**翻译成执行规则**：
- 只抽色值 + 字体、不找 logo / 产品图 / UI / mascot → **违反本协议**
- 用 CSS 剪影/SVG 手画替代真实产品图或 mascot → **违反本协议**（生成的就是「通用动画」，任何品牌都长一样）
- 找不到资产不告诉用户、也不 AI 生成，硬做 → **违反本协议**
- 宁可停下问用户要素材，也不要用 generic 填充

##### 6 步硬流程（每步有 fallback，绝不静默跳过）

##### Step 0 · 先扫工作目录（最便宜，最常被跳过 · 永远先做）

**视频/动画制作场景的硬要求**：影片是品牌物料里**识别度门槛最高**的形式——声画俱全，不专业感会被放大十倍。所以：

> **任何品牌动画/影片任务，开工第一件事必须是扫工作目录与父目录**。优先级高于 WebSearch、高于 WebFetch、高于问用户。

**典型场景**：用户的 OneDrive / Documents / 项目目录里，常已有设计师交付或收购品牌带来的官方品牌资产资料夹（含 logo / mascot / brand spec PDF/HTML / 色票），但 agent 跳过这步直接 WebFetch 抓官网 banner 图当作品牌资产，导致后续 Step 全部建立在错误前提上。

**硬流程**：

1. `ls -la` 当前工作目录、父目录
2. 搜常见品牌资产文件夹命名（中英文都查）：
   - **英文**：`brand-identity/` · `BrandIdentity/` · `brand-spec/` · `brand-system/` · `design-system/` · `assets/` · `brand/` · `uploads/` · `<brand>-brand/` · `<brand>-assets/`
   - **中文**：`品牌視覺系統` · `品牌規範` · `品牌識別系統` · `品牌識別` · `視覺規範` · `VI` · `CI` · `品牌指南`
3. 找到资料夹后**全部展开 `ls -la`**，注意以下文件：
   - `Brand Identity System.html` / `brand-guideline.pdf` / `brand-spec.md` / `README.md`
   - `*.png` `*.svg`（logo / mascot / 配图）
   - `uploads/` 子资料夹（用户常把素材丢这里）
4. 用 `Read` 打开 logo / mascot 图片**视觉确认**——不要只看文件名（文件名只能确认存在性，不能确认它长什么样、是不是你预期的物种/造型）
5. 读完整 brand spec HTML/PDF 抽取色票、字体、精神三轴、禁区
6. 找不到任何资产再进 Step 1（问用户）；找到了就只问「还有补充资料吗？」

**为什么这步必须放在「问用户」之前**：

- 用户可能没意识到资料夹里有官方品牌资产（OneDrive 同步来的、设计师交付的、之前对话留下的、收购品牌带来的）
- 直接「问用户有没有」常会得到「应该没有吧」（因为他自己也忘了）
- 但 `ls` 5 秒能直接看到实情
- 跳过这步 = 后面所有 Step 都建立在错误前提上
...
# 搜本机所有与品牌名相关的资料夹（按平台调整路径）
find "$HOME/OneDrive" -maxdepth 4 -iname "*<brand>*" 2>/dev/null
find . -iname "*brand*" -o -iname "*品牌*" -o -iname "*VI*" 2>/dev/null | head -20
...
**自检（开工前 10 秒）**：
- [ ] 我 `ls` 过工作目录和父目录了吗？
- [ ] 工作目录是否有 `*品牌*` / `*brand*` / `*VI*` / `*视觉*` 资料夹？
- [ ] 我 `Read` 视觉确认过 logo/mascot 文件了吗？（不是只看文件名）
- [ ] 我读完 brand spec HTML/PDF 抽出色票了吗？

任何一项 No → **立即停下回到 Step 0**。
...
- **某宠物清洁品牌动画（2026-05-03，触发 Step 0 + Mascot 警示新增）**：用户工作目录同级已有完整官方品牌视觉系统资料夹（含 Brand Identity System HTML + uploads/ 含 mascot.png/logo-light.png/logo-dark.png + 官方 OKLCH 色票表 + 品牌精神三轴），但开工跳过 Step 0 没扫目录，直接 WebFetch 抓官网 banner 图（logo 仅嵌在 jpg 角落）。结果：(1) 手画了一个完全不同物种的 SVG mascot（官方 mascot 是日本达摩 × 柴犬，agent 画成红熊抱木桶）。(2) 主品牌色用了「接近但不是」的 hex。(3) 品牌名漏字。三处错误同时发生，做了两版才被用户指出。**根因：跳过 Step 0 工作目录扫描 + 凭推测 SVG 手画 mascot**。→ Step 0 + Mascot 警示加入协议。
