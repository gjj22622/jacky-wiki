...
> **跨工具補充（2026-06-24）**：本案用 Claude Code，但「先計畫後執行」三大工具軌都有。Codex 有 plan mode；**Google Antigravity 有 Planning Mode（相對 Fast Mode）**，會先產一份 Implementation Plan artifact（目標／技術棧／步驟／檔案變更／測試），你用 Google-Docs 式畫線留言改、回「proceed」核准後它才逐步自走，最後再產 Walkthrough artifact（變更摘要＋怎麼測）。心法一致：大事先 plan、小事直接做。完整對照見 [aischool / Plan 模式與自動化工作流](../../../aischool/concepts/Plan模式與自動化工作流.md)。
