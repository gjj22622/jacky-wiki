...
## 2026-06-27 — AI School 新增 L5-10「Plugin 外掛包」單元（42→43 課）

**做了什麼**：平台課程實際新增一支影片單元 L5-10 Plugin 外掛包（進階選修，與 MCP/CLI 同群），edge-tts 雲哲版渲染並上線（Zeabur）。wiki 同步：課程地圖表插 L5-10（#36）並把 L6 重編號到 37-43、各處 42→43 課、92分55秒→95分35秒、L5 章節頁 9→10 課並加 L5-10 條目。

**為什麼**：Jacky 發現 MCP/CLI 有單元但 Plugin 完全沒教（缺口）。MCP/CLI 仍是「概念＋淺講、深度留工程階段」；Plugin 補成完整一支（Skill vs Plugin、市集安裝、双云兩個使用實例、來源信任）。

**決策**：① 此 L5-10 是**真影片單元**（有 final.mp4），與 2026-06-24 log 那筆「不新增 L5-10 影片課」不衝突——那筆講的是 Plan 模式（無影片、放 concepts/），這支 Plugin 有影片故入 curriculum 鏡射 course.json。② build-course-json 改：無 final.mp4 的單元 video 設 null（前台顯示「影片尚未上傳」）。③ 影片先用基礎 TTS 上線，克隆聲音待換（配音 backlog）。

---

## 2026-06-25 — 七大外部 Agent 平台評估 + n8n/LangSmith 落地計畫

**做了什麼**：J董丟來業界主流 Agent 平台清單（Vertex / LangSmith / CrewAI / Cloud Run / Bedrock / Zapier / n8n）請 wiki 協助評估。對照 ailab 工具棧 + jos 作業系統觀後定調，並落地四處：
- 新建 `ailab/experiments/n8n-langsmith落地計畫.md`（分軌道 A/B、分階段試點、升格/關檔判準）。
- `ailab/tools/工具棧索引.md`「觀望中」補 LangSmith、強化 n8n 理由、加「不採用五個」註記。
- `jos/作業系統索引.md` 補設計決策「為什麼不採用外部 Agent 平台」。
- `ailab/experiments/2026-Q2實驗清單.md` 加 EXP-06（n8n）、EXP-07（LangSmith）。

**為什麼**：jos 域確立後，評估軸從「補缺口」升級為「裝進我的 OS vs. 搬進別人的封閉 OS」。七個裡只有 n8n（開源可攜，符合 jos）與 LangSmith（SaaS 觀測層，低耦合）通過；其餘五個本質是綁雲廠商或拆自建資產。

**留下決策**：
- 唯一判準：**裝進 jos（可 /移機 還原）vs. 搬進別人 OS（換機回不來）**。
- n8n 走軌道 A 優先（已在觀望清單，低風險）；LangSmith 走軌道 B 條件性（双云客戶量爆黑盒子才投，先驗非 LangChain 接合成本）。
- 例外條款：若接到指定 GCP/AWS 的企業客戶，再就該案單獨評估，不改自身 OS 預設。

**頁數**：ailab experiments +1（落地計畫）；總 242 → 243。
...
- **內容焦點**：以 Jacky Wiki 為案例，教「如何建立自己的知識圖譜 wiki」（方法論）——而非介紹 Jacky Wiki 本身（那是 `團隊分享簡報.md` 的工作）
...
- `wiki/團隊分享.md`：給双云夥伴／TBSA 學員的 wiki 介紹文件（2026-06-25 已併入 `wiki/團隊分享簡報.md`）
- `wiki/團隊分享簡報.md`：Marp 格式簡報（可直接 export PPTX/PDF），現為唯一保留的團隊分享稿
