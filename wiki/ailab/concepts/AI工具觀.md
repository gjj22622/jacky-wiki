...
## deterministic 先行，LLM 只在分岔點（90/9/1）⭐

> 2026-06-23 從 AEO/loop 實作沉澱：別讓 LLM 包整條流程，**把 LLM 關進「只有判斷題／只有偵測到變動」才呼叫的閘**。

- **90% deterministic 程式**：編排、抓取、文字 diff、路由、渲染、回流——能用程式確定性做的，全用程式，**零 LLM 成本**。
- **9% LLM**：只在「真的需要語言判斷」的分岔點呼叫（解析變動、生內容、排序）。且每個 LLM 點都被**程式守門包夾**（預算熔斷、邊界掃描、紅線 guard、自動重抽）。
- **1% 人**：只在不可逆／對外／改碼的最後一哩點頭（Telegram /yes）。
- **省成本心法**：規則 loop「**只有偵測到變動才動 LLM**」、撰稿「答案先行先用程式檢」、體檢「技術分走確定性 scorer 零幻覺」——平時零成本空轉，LLM 只在值得的地方燒。
- 自我提示：設計任何 AI 流程先問——**這步真的需要 LLM 嗎？還是 curl/diff/規則就能做？**

## 相關連結

- 工具棧 → [工具棧索引](../tools/工具棧索引.md)
- 模型分流 → [模型選擇心法](../tools/模型選擇心法.md)
- 模式 → [[自我進化規則loop]]（只有變動才動 LLM 的實作）、[[指標雜訊底先測]]
