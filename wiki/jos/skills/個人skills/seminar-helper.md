...
## 🔴 核心同步規則（最容易遺漏，每次動檔前先看）

### 規則 A：四件套必須即時同步

專討核心產出**四件套**：**簡報（.pptx）／摘要（.docx）／講稿口稿（.md）／音訊（.mp3）**。

**動其中一個 → 必須查另外三個是否要跟改**。

| 動了 | 一定要檢查 | 為什麼 |
|---|---|---|
| **簡報** 改數字 / 文字 / 引用 | **摘要、講稿、音訊** 全部要看 | 上場時三者並用，不一致會被當場抓 |
| **摘要** 改數字 / 文字 | **簡報、講稿** 要對照（音訊看是否值得重生）| 摘要是交件版、不能跟現場講不同 |
| **講稿** 改文字 | **音訊**（要不要重生 TTS）、**簡報**（口白對應正確）| 講稿是音訊的源頭 |
| **音訊**（TTS）| 不會獨立改，只會因講稿改而重生 | — |

**血淚教訓**（第三次專討 2026-05-13 上場日）：V5 簡報修正 50%/85% → 80%/40% 後**忘了同步檢查摘要**，摘要還停在 50%/85% 錯誤版，差點上場後被抓包。

**SOP**：寫一個 docstring 風格的修改紀錄，每次動四件套之一就寫：
```
修改：YYYY-MM-DD 改 {哪份檔} 的 {什麼內容}
同步檢查：[ ] 簡報 [ ] 摘要 [ ] 講稿 [ ] 音訊
→ 必須全部勾完才算結束
```
...
任何檔案產生新版本（V5 → V6、v1 → v2、修正版、含老師回饋版等），**舊版立刻搬到「舊文件 / _歸檔」資料夾**，不要留在主資料夾。

**搬法**：
- 同層建 `舊文件/` 子資料夾，或
- 用既有的 `_歸檔_YYYY-MM-DD/` 命名

**標準動作**：
1. 新版另存 → 命名加版本號（如 `_v5_鐘基啟_`）
2. 舊版搬到 `舊文件/`（保留檔名不動）
3. 主資料夾**只留最新版**

**為什麼**：專討準備一輪會產生 30-50 個檔（簡報 V1-V6、講稿 V3-V6、摘要 v2-v5、音檔 V5/V6 兩份、PDF 備份⋯⋯），主資料夾爆滿後**找最新版會找錯**。第三次專討的 `舊文件/` 已塞滿 v2/v3/v3_盟元改 等多個版本，就是這個原則的實踐。

**例外**：備份檔（`.bak_YYYY-MM-DD_pre_*.docx`）可以留在主資料夾旁，但要有日期+原因後綴；不歸檔的話**至少 30 天內要清掉**。
...
**準備任何新專討時的第一動作**：讀 `C:\Users\gjj22\jacky-wiki\wiki\nchu\Q&A預備池.md`，掃看歷次專討被老師追問、答不好的問題。**主動向 Jacky 提醒**：
> 「我看到你準備第 X 次專討。先翻一下 `wiki/nchu/Q&A預備池.md`，第三次有 X 題沒答好，這次先把答案內化進新講稿。」

Jacky 自承會忘半年前的事，所以 AI 要**主動提**，不是等他問。
...
14. **四件套必須同步**：簡報改 → 摘要、講稿、音訊都要連動檢查（見頂部「核心同步規則 A」）。第三次專討當天差點被 50%/85% 數字打臉，原因是 V5 簡報已改但摘要漏改。
15. **新版完成立刻歸檔舊版**：主資料夾**只能有最新版**，舊版搬 `舊文件/`（見頂部「核心同步規則 B」）。不歸檔會導致下次開檔開到舊版上場炸鍋。
16. **學術引述措辭**：講稿 / 摘要 / 教學包引用文獻**禁止用「有一篇」「有一篇 XXXX 年論文」**這類口語句型；必須用「**XXXX 年，XX 等人發表的文獻中指出**」（XX 是第一作者姓氏）。詳見 `memory/feedback_academic_citation_phrasing.md`。
17. **簡報 in-text 引用不寫 initial**：投影片內、圖表 footer 的 in-text 引用一律寫「**Tian et al. 2023**」不寫「Tian, L. et al. 2023」。同姓不同人才保留 initial 區別（例：Tian L vs Tian X）。文末參考文獻清單則維持 Zool Stud 完整 initial 格式（規則 #5）。詳見 `memory/feedback_slide_intext_citation_no_initial.md`。
