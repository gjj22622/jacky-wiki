...
## 2026-07-01（新增週期性任務自動化 workshop）

**背景**：為 airun.tw 建「每月 1 號自動營運月報」的實作對話，Jacky 要把這套週期性自動化做法整理成內訓教材給同事。

**決策**：
- 走 **workshop 模式**（對外成品給內部團隊），不是 ailab（個人事件已另捕在 `ailab/inbox/2026-07-01-週期性自動化報告的正確架構.md`）。
- 教材聚焦三個可複用核心，而非流水帳：**排程選型（雲端 routine 讀不到 gitignored 機密 → 本機 systemd timer）／90-9-1 拆分（AI 只合成不生數字）／密鑰活性探針（防通知閘 401 靜默斷）**。
- 用**相對路徑連結**引用 ailab 既有結晶（AI 作業系統觀、自我進化規則 loop、claude-code 工具頁），不複製內容（守去重原則）。

**新增頁面**：education/workshops/2026-07-01-週期性任務自動化/{README.md, slides-outline.md}（+2）。education 域 41→43。
